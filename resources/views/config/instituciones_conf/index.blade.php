			...
			@foreach ($institucion_configs as $institucion_config)
				<tr>
				    ...
				    <td><a href="{{ route('config.instituciones_conf.edit', $institucion_config->id ) }}" class="btn btn-warning"><span class="glyphicon glyphicon-wrench" aria-hidden= "true"></span></a>
                     ...
                     <a href="{{ route('config.instituciones_conf.destroy', $institucion_config->id ) }}" class="btn btn-danger" onclick="return confirm('Seguro que desea eliminar la configuración?')"><span class="glyphicon glyphicon-remove-circle" aria-hidden= "true"></span> </a>
